...
[[maybe_unused]] static void printIncoming(const Bytes &msg) {
  ...
  if (auto dp = decodeDirectProgram(msg); dp.valid) {
    std::printf(
        "    Direct Program: pos=0x%X mask=0x%02X shift=%u data=0x%02X\n",
...
struct MidiBackend {
  ...
  static void listPorts() {
    ...
    for (ItemCount i = 0; i < MIDIGetNumberOfDestinations(); ++i)
      std::printf("  [%lu] %s\n", i,
                  endpointName(MIDIGetDestination(i)).c_str());
  ...
  static int autoPick(bool dest) {
    ItemCount n =
        dest ? MIDIGetNumberOfDestinations() : MIDIGetNumberOfSources();
    for (ItemCount i = 0; i < n; ++i) {
      auto ep = dest ? MIDIGetDestination(i) : MIDIGetSource(i);
      if (lower(endpointName(ep)).find("sid") != std::string::npos)
        return (int)i;
    }
    return n > 0 ? 0 : -1;
  }
  ...
  bool open(int outIdx, int inIdx) {
    ...
    MIDIInputPortCreate(client, CFSTR("sidprobe in"), readProc, nullptr,
                        &inPort);

    if (outIdx < 0)
      outIdx = autoPick(true);
    if (inIdx < 0)
      inIdx = autoPick(false);
  ...
  void send(const Bytes &bytes) {
    ...
    pkt = MIDIPacketListAdd(list, sizeof buffer, pkt, 0, bytes.size(),
                            bytes.data());
    if (!pkt) {
      std::fprintf(stderr, "message too large for packet list\n");
      return;
    }
    OSStatus st = MIDISend(outPort, dest, list);
    if (st != noErr)
      std::fprintf(stderr, "MIDISend error %d\n", (int)st);
  }
};
...
struct MidiBackend {
  ...
  static void listPorts() {
    std::printf(
        "(dry-run build: no real MIDI ports - CoreMIDI is macOS-only)\n");
  }
...
static void listParams(const std::string &filter) {
  ...
  for (const auto &p : parameters()) {
    if (!filter.empty() && lower(p.id).find(lower(filter)) == std::string::npos)
      continue;
    std::printf("  %-24s %-20s range %d..%d%s\n", p.id.c_str(), p.group.c_str(),
                p.minValue, p.maxValue,
                p.cc >= 0 ? (" cc=" + std::to_string(p.cc)).c_str() : "");
    ++count;
  }
...
static void printHelp() {
  std::printf(
      ...
      "  params [filter]            list parameter ids (optional substring "
      "filter)\n"
      ...
      "  raw  <hex> [hex...]        send raw bytes, e.g. raw F0 00 20 3C 01 00 "
      "03 F7\n"
      ...
      "  help                       this help\n"
      "  quit                       exit\n"
      "\nTip: dump a patch FROM the unit using its front panel, incoming SysEx "
      "is\n"
      "decoded and captured automatically, then `save before.syx`. (Patch "
      "all-clear\n"
      "is intentionally omitted, it wipes patch memory, use `raw` if you "
      "really\n"
      "need it.)\n");
}
...
static bool runCommand(MidiBackend &midi, const std::vector<std::string> &tk) {
  ...
  if (cmd == "list") {
    MidiBackend::listPorts();
    return true;
  }
  if (cmd == "params") {
    listParams(tk.size() > 1 ? tk[1] : "");
    return true;
  }

  if (cmd == "send" || cmd == "cc") {
    if (tk.size() < 3) {
      std::printf("usage: %s <id> <value>\n", cmd.c_str());
      return true;
    }
    const ParamInfo *p = findParamById(tk[1]);
    if (!p) {
      std::printf("Unknown parameter '%s' (try: params %s)\n", tk[1].c_str(),
                  tk[1].c_str());
      return true;
    }
    int v;
    ...
    if (cmd == "send") {
      ...
      if (p->cc < 0) {
        std::printf("'%s' has no MIDI CC; use `send`.\n", p->id.c_str());
        return true;
      }
  ...
  if (cmd == "note" || cmd == "off") {
    if (tk.size() < 2) {
      std::printf("usage: %s <note> ...\n", cmd.c_str());
      return true;
    }
    int note, vel = 100, ch = 0;
    if (!parseInt(tk[1], note)) {
      std::printf("Bad note\n");
      return true;
    }
    if (cmd == "note") {
      ...
      if (tk.size() > 3)
        parseInt(tk[3], ch);
      Bytes msg{static_cast<Byte>(0x90 | (ch & 0x0F)),
                static_cast<Byte>(note & 0x7F), static_cast<Byte>(vel & 0x7F)};
      ...
      if (tk.size() > 2)
        parseInt(tk[2], ch);
      Bytes msg{static_cast<Byte>(0x80 | (ch & 0x0F)),
                static_cast<Byte>(note & 0x7F), 0};
  ...
  if (cmd == "save") {
    if (tk.size() < 2) {
      std::printf("usage: save <path.syx>\n");
      return true;
    }
    std::lock_guard<std::mutex> lock(g_rxMutex);
    if (g_rxAll.empty()) {
      std::printf("nothing captured yet\n");
      return true;
    }
...
int main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    std::string a = argv[i];
    if (a == "--out" && i + 1 < argc)
      parseInt(argv[++i], outIdx);
    else if (a == "--in" && i + 1 < argc)
      parseInt(argv[++i], inIdx);
    else
      oneShot.push_back(a);
  }

  // `sidprobe list` should work without opening ports.
  if (oneShot.size() == 1 && oneShot[0] == "list") {
    MidiBackend::listPorts();
    return 0;
  }

  MidiBackend midi;
  if (!midi.open(outIdx, inIdx))
    return 1;
  ...
  while (true) {
    ...
    if (!std::getline(std::cin, line))
      break;
    if (!runCommand(midi, tokenize(line)))
      break;
  }
  return 0;
}
