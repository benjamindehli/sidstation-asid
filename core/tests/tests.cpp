...
// Builds the expected 12-byte DP message from the manual's "posHi posLo mask
// shift data" pair.
static Bytes dp(Byte pHi, Byte pLo, Byte mask, Byte shift, Byte data) {
  return {0xF0, 0x00, 0x20, 0x3C,  0x01, 0x00, 0x04,
          pHi,  pLo,  mask, shift, data, 0xF7};
}

static void testDirectProgramFraming() {
  // Manual example (page 43): "DP: 01 76 0f 04 08" - first table position =
  // Noise.
  DpAddress tablePos{static_cast<std::uint16_t>((0x01 << 7) | 0x76), 0x0F,
                     0x04};
  ...
  CHECK(d.address.mask == 0x0F && d.address.shift == 0x04,
        "DP decode mask/shift");
...
// Each entry: parameter id, and the manual's documented "posHi posLo mask
// shift".
struct Known {
  const char *id;
  Byte pHi, pLo, mask, shift;
};
...
static void testParamAddresses() {
  ...
  for (const auto &k : known) {
    ...
    if (!p) {
      ++g_checks;
      ++g_failures;
      std::printf("FAIL: missing param %s\n", k.id);
      continue;
    }
    Byte data = encodeParamValue(*p, p->minValue);
    checkBytes((std::string("DP addr ") + k.id).c_str(),
               encodeDirectProgram(p->dp, data),
               dp(k.pHi, k.pLo, k.mask, k.shift, data));
  }
}
...
static void testBipolarEncoding() {
  ...
  CHECK(encodeParamValue(*tr, -24) == static_cast<Byte>(-24 & 0x7F),
        "transpose -24");
  CHECK(decodeParamValue(*tr, 0x7F) == -1, "decode 0x7F -> -1");

  const ParamInfo *dt = findParamById("osc1.detune");
  CHECK(decodeParamValue(*dt, encodeParamValue(*dt, -64)) == -64,
        "detune -64 round-trip");
  CHECK(decodeParamValue(*dt, encodeParamValue(*dt, 63)) == 63,
        "detune +63 round-trip");
}

static void testCcMap() {
  // Spot-check CC assignments from pages 38-39.
  CHECK(findParamByCc(27) && findParamByCc(27)->id == "filter.cutoff",
        "CC27 -> filter cutoff");
  CHECK(findParamByCc(24) && findParamByCc(24)->id == "osc1.active",
        "CC24 -> osc1 active");
  CHECK(findParamByCc(70) && findParamByCc(70)->id == "osc2.sustain",
        "CC70 -> osc2 sustain");
  CHECK(findParamByCc(102) && findParamByCc(102)->id == "lfo2.addDepth",
        "CC102 -> lfo2 add depth");
  CHECK(findParamByCc(117) && findParamByCc(117)->id == "lfo4.laceSpeed",
        "CC117 -> lfo4 lace speed");
  ...
  checkBytes("CC message cutoff=64", controlChange(0, *cutoff, 64),
             Bytes{0xB0, 27, 64});
  ...
  CHECK(ccValue(*active, 1) == 127 && ccValue(*active, 0) == 0,
        "bool -> 0/127");

  // A sub-range param scales up onto 0..127.
  const ParamInfo *attack = findParamById("osc1.attack"); // range 0..15
  CHECK(ccValue(*attack, 15) == 127 && ccValue(*attack, 0) == 0,
        "0..15 scales to 0..127");
  ...
  CHECK(ccValue(*tr, 24) == 127 && ccValue(*tr, -24) == 0,
        "transpose extremes map to 0/127");
}
...
static void testPatchRoundTrip() {
  ...
  for (std::size_t i = 10; i < p.data.size(); ++i)
    p.data[i] = static_cast<Byte>((i * 7 + 3) &
                                  0xFF); // arbitrary but full 8-bit values
  ...
  CHECK(encodeSkipPatch() ==
            (Bytes{0xF0, 0x00, 0x20, 0x3C, 0x01, 0x00, 0x03, 0xF7}),
        "skip patch bytes");
}
...
static void testRegistrySanity() {
  ...
  for (const auto &p : t) {
    ...
    CHECK(p.maxValue >= p.minValue,
          (std::string("range order: ") + p.id).c_str());
...
static void testEnumChoices() {
  ...
  if (wave) {
    ...
    CHECK(wave->choices.back().value == 8 &&
              wave->choices.back().label == "Noise",
...
static void testSysExAssembler() {
  ...
  if (whole.size() != 2) {
    ++g_failures;
    std::printf("FAIL: assembler count %zu\n", whole.size());
  } else {
  ...
  for (auto &m : b.feed(stream.data(), cut))
    acc.push_back(m);
  for (auto &m : b.feed(stream.data() + cut, stream.size() - cut))
    acc.push_back(m);
  CHECK(acc.size() == 2, "assembler across chunks: 2 messages");
  if (acc.size() == 2)
    checkBytes("chunked msg2", acc[1], msg2);
}
...
static void testSyxAndLibrary() {
  ...
  if (items.size() == 2) {
    ...
    CHECK(solo && solo->name() == "Beta",
          "item message is a valid single dump");
    CHECK(items[0].message == encodePatchDump(alpha),
          "item message bytes exact");
  }
  ...
  CHECK(!readSyxFile((dir / "nope.syx").string()).has_value(),
        "readSyxFile missing");
  CHECK(!loadPatchFromFile((dir / "notes.txt").string()).has_value(),
        "load non-patch");
...
static void testVoiceEngine() {
  ...
  CHECK(!backTo60[0].retrigger,
        "legato fall-back retunes without re-attacking");
  ...
  CHECK(off62.size() == 1 && !off62[0].gateOn,
        "releasing active note gates off");
...
static void testAsid() {
  ...
  checkBytes("ASID LCD", encodeAsidLcd("Hi"),
             Bytes{0xF0, 0x2D, 0x4F, 'H', 'i', 0xF7});
  ...
  // Update with two low registers (voice 1 frequency): slots 0 and 1 -> mask1 =
  // 0x03.
  ...
  // A value with the 8th bit set must show up in the msb byte, data carries 7
  // bits. reg 0x16 is slot 19 -> byte 2, bit 5 (mask/msb byte index 2 = 0x20).
  checkBytes("ASID update msb bit", encodeAsidUpdate({{0x16, 0xC0}}),
  ...
  CHECK(sidFrequency(69) >= 7492 && sidFrequency(69) <= 7494,
        "A4 -> ~7493 SID freq");
  CHECK(sidFrequency(69 + 12) > sidFrequency(69) * 1.9,
        "octave up roughly doubles freq");
  ...
  CHECK(s.reg[4] == (sid::kPulse | sid::kGate),
        "voice1 control = pulse + gate");
  CHECK(s.fullUpdate().front() == 0xF0 && s.fullUpdate().back() == 0xF7,
        "full update framed");
}
...
static void testAsidPlayer() {
  ...
  auto s = p.start();
  CHECK(s.size() == 1,
        "start is a single register-setup update (no ASID start cmd)");
  CHECK(!s.empty() && s[0].front() == 0xF0 && s[0].back() == 0xF7,
        "start update framed");
  ...
  CHECK(
      (frames[0][6] & ((1 << 1) | (1 << 4))) == ((1 << 1) | (1 << 4)),
      "note on double-writes the control register (primary + secondary slot)");
  CHECK((frames[1][6] & (1 << 4)) == 0,
        "flush uses only the primary control slot, no second edge");
  ...
  CHECK(offFrames.size() == 2,
        "note off is the double-control release plus a flush");
  ...
  // Target-voice mode: every note goes to the chosen voice regardless of
  // channel.
  AsidVoicePlayer p2;
  p2.setTargetVoice(2);  // voice 3, control register index 18
  p2.noteOn(0, 60, 100); // channel 0, but forced to voice 2
  CHECK((p2.state().reg[18] & sid::kGate) != 0,
        "target voice 2 gated on from any channel");
  CHECK((p2.state().reg[4] & sid::kGate) == 0,
        "voice 0 untouched in target mode");
  p2.noteOff(0, 60);
  ...
  CHECK((p.state().reg[24] & 0x0F) == 10,
        "volume stored in low nibble of reg 0x18");
  ...
  CHECK(p.state().reg[6] == ((12 << 4) | 2),
        "sustain/release nibbles in reg 6");
  p.setPulseWidth(0, 0x0800);
  CHECK(p.state().reg[2] == 0x00 && p.state().reg[3] == 0x08,
        "pulse width split 12-bit");
  p.setFilterRouting(0, true);
  CHECK((p.state().reg[23] & sid::kFilt1) != 0,
  ...
  CHECK((p.state().reg[23] & 0x0F) == 0x05,
        "full routing bits written together");
  p.setWaveform(0, sid::kPulse);
  CHECK((p.state().reg[4] & 0xF0) == sid::kPulse,
        "waveform bits set on control register");
  p.setSync(0, true);
  p.setRing(0, true);
  CHECK((p.state().reg[4] & sid::kSync) != 0 &&
            (p.state().reg[4] & sid::kRing) != 0,
        "sync and ring bits set");
  CHECK((p.state().reg[4] & 0xF0) == sid::kPulse,
        "sync/ring keep the waveform bits");
  p.setSync(0, false);
  CHECK((p.state().reg[4] & sid::kSync) == 0 &&
            (p.state().reg[4] & sid::kRing) != 0,
  ...
  CHECK(!pm.setPitchMod(0, 12.0).empty(),
        "pitch mod writes frequency while a note sounds");
  CHECK(freq0() > baseFreq, "an octave up raises the frequency value");
  pm.noteOff(0, 60);
  CHECK(pm.setPitchMod(0, 5.0).empty(),
        "pitch mod stops once the note is released");
  ...
  CHECK(freqUp > freqPlain * 3 / 2,
        "coarse tune up raises the played frequency (about double an octave)");
  CHECK(!tune.setPitchMod(0, 0.0).empty() &&
            (tune.state().reg[0] | (tune.state().reg[1] << 8)) == freqUp,
        "the tune offset folds into pitch modulation too");
}

// setPitchTo writes a frequency for an explicit note with no note held, which
// is what lets the release tail keep its vibrato instead of freezing detuned.
static void testPitchTo() {
  ...
  CHECK(!p.setPitchTo(0, 69.0).empty(),
        "setPitchTo works with no note sounding");

  const std::uint16_t a4 = sidFrequency(69.0);
  CHECK(p.state().reg[0] == (a4 & 0xFF) &&
            p.state().reg[1] == ((a4 >> 8) & 0xFF),
  ...
  const std::uint16_t half =
      static_cast<std::uint16_t>(p.state().reg[0] | (p.state().reg[1] << 8));
  CHECK(half > a4 && half < sidFrequency(70.0),
        "a fractional note lands between semitones");
  ...
  p.noteOn(0, 60, 100);
  p.setPitchMod(0, 3.0);
  const std::uint16_t viaMod =
      static_cast<std::uint16_t>(p.state().reg[0] | (p.state().reg[1] << 8));
  p.setPitchTo(0, 63.0);
  const std::uint16_t viaTo =
      static_cast<std::uint16_t>(p.state().reg[0] | (p.state().reg[1] << 8));
  CHECK(viaMod == viaTo, "setPitchMod(+3) matches setPitchTo(note + 3)");
}

// The hard restart is what dodges the 6581 ADSR delay bug: it must force the
// release nibble to 0 with the gate held low so the envelope drains, WITHOUT
// disturbing the stored registers, so the following note-on writes the player's
// real release back.
static void testHardRestart() {
  AsidVoicePlayer p;
  CHECK(p.hardRestartFrames(-1).empty(),
        "hard restart rejects a negative voice");
  ...
  // Frame 1 writes SR (reg 6 -> slot 5, mask/msb byte 0 bit 5 = 0x20), value
  // 0xF0. Frame 2 rewrites control (reg 4 -> slot 22, byte 3 bit 1 = 0x02),
  // value 0x20.
  auto hr = p.hardRestartFrames(0);
  CHECK(hr.size() == 2,
        "hard restart is two frames: fast release, then the commit");
  checkBytes("hard restart frame 1 (sustain kept, release 0)", hr[0],
             Bytes{0xF0, 0x2D, 0x4E, 0x20, 0x00, 0x00, 0x00, 0x20, 0x00, 0x00,
                   0x00, 0x70, 0xF7});
  checkBytes("hard restart frame 2 (control, gate low)", hr[1],
             Bytes{0xF0, 0x2D, 0x4E, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00,
                   0x00, 0x20, 0xF7});

  // With a real envelope: sustain 9, release 12 -> reg 6 = 0x9C, and the frame
  // must carry 0x90, i.e. the sustain nibble kept and the release nibble
  // zeroed.
  ...
  CHECK(p.state().reg[6] == 0x9C, "hard restart does not touch the stored "
                                  "envelope, so the note-on restores it");

  // While a note sounds the gate is high; the drain frame must still gate low,
  // and again leave the stored control register alone.
  p.noteOn(0, 69, 100);
  CHECK((p.state().control(0) & sid::kGate) != 0,
        "note on leaves the gate high");
  hr = p.hardRestartFrames(0);
  CHECK((hr[1][11] & sid::kGate) == 0,
        "hard restart drops the gate in the drain frame");
...
static void testLfo() {
  ...
  CHECK(std::abs(lfo.phase() - 0.25) < 1e-9,
        "advance wraps the phase into 0..1");

  // setPhase takes the song position in CYCLES, so 1.05 is "just into cycle 1".
  // Sample & Hold holds one value per cycle and takes a new one at each
  // boundary.
  ...
  lfo.setPhase(0.4);
  CHECK(std::abs(lfo.value() - a) < 1e-12,
        "sample & hold holds its value within a cycle");
  lfo.setPhase(1.05); // into the next cycle
  const double b = lfo.value();
  CHECK(std::abs(b - a) > 1e-12,
        "sample & hold picks a new value on a cycle boundary");
  CHECK(b >= -1.0 && b <= 1.0, "sample & hold stays bipolar");

  // A backward jump is the transport looping, not a new cycle: going back
  // inside an earlier cycle must not re-roll the value, which is what made a
  // looped bar sound different on every pass.
  lfo.setPhase(0.5); // loop back into cycle 0
  CHECK(std::abs(lfo.value() - b) < 1e-12,
        "a backward jump does not re-roll sample & hold");
  lfo.setPhase(1.5); // forward across the boundary again
  CHECK(std::abs(lfo.value() - b) > 1e-12,
        "crossing forward again does advance it");
  ...
  lfo.setPhase(0.5);
  CHECK(std::abs(lfo.value() - g0) > 1e-9,
        "random glides within a cycle, it does not hold");
  lfo.setPhase(0.999);
  const double gEnd = lfo.value();
  lfo.setPhase(1.001); // boundary: rndFrom becomes the previous rndTo
  CHECK(std::abs(gEnd - lfo.value()) < 0.05,
        "random is continuous across the boundary");
  CHECK(lfo.value() >= -1.0 && lfo.value() <= 1.0,
        "random glide stays bipolar");

  // A step covering several cycles must advance the endpoints once per cycle,
  // not once per call. Reachable when the modulation clock resumes after a gap
  // (dt is clamped to 4x its interval) at a high rate. Compare against stepping
  // one cycle at a time: same number of boundaries must give the same value.
  Lfo fast, slow;
  ...
  for (int i = 0; i < 3; ++i)
    slow.advance(0.05, 20.0); // 1 cycle at a time, 3 times
  CHECK(std::abs(fast.phase() - 0.2) < 1e-9,
        "a multi-cycle step lands on the right phase");
  ...
  CHECK(std::abs(still.phase()) < 1e-12,
        "advancing by nothing does not move the phase");
  still.advance(0.1, -5.0);
  CHECK(std::abs(still.phase()) < 1e-12,
        "a negative rate does not move the phase backwards");
}
...
static void testWaveformBits() {
  using namespace sid;
  // Bits OR together.
  CHECK(waveformBits(false, false, false, false) == 0,
        "no waveform selected is silent");
  ...
  CHECK(waveformBits(true, true, false, false) == (kTriangle | kSaw),
        "triangle + saw combine");
  CHECK(waveformBits(true, true, true, false) == (kTriangle | kSaw | kPulse),
        "tri + saw + pulse combine");
  // Noise is exclusive: it wins alone, whatever else is set.
  CHECK(waveformBits(false, false, false, true) == kNoise, "noise only");
  CHECK(waveformBits(true, true, true, true) == kNoise,
        "noise locks out the other waveforms");
  CHECK(waveformBits(true, false, false, true) == kNoise,
        "noise beats triangle");
}
