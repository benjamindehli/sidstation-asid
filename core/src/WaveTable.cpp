...
void WaveTablePlayer::configure(int length, int loopPoint, int speed) {
    ...
    if (length_ == 0) loop_ = 0;
    else loop_ = loopPoint < 0 ? 0 : (loopPoint >= length_ ? length_ - 1 : loopPoint);
    if (pos_ >= length_) pos_ = loop_;  // a shrunk table must not point past the end
    if (active_ && length_ == 0) active_ = false;
}
