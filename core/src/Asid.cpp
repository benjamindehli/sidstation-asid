...
// voice control registers (4, 11, 18) sit at ids 21..23... note ids 22/23/24
// map to 4/11/18, and 25/26/27 are their secondary slots for double writes.
...
int asidSlotForRegister(Byte reg) {
  ...
  for (int id = 0; id < 25; ++id)
    if (kSlotToReg[id] == reg)
      return id;
  return -1;
}

Bytes encodeAsidStart() {
  return {sysex::kStart, kAsidManId, kCmdStart, sysex::kEnd};
}
Bytes encodeAsidStop() {
  return {sysex::kStart, kAsidManId, kCmdStop, sysex::kEnd};
}

Bytes encodeAsidLcd(const std::string &text) {
  Bytes out{sysex::kStart, kAsidManId, kCmdLcd};
  for (char c : text)
    out.push_back(static_cast<Byte>(c) & 0x7F);
...
static Bytes encodeFromSlots(const Byte *value, const bool *present) {
  ...
  for (int id = 0; id < 28; ++id) {
    ...
    if (value[id] & 0x80)
      msb[byteIndex] = static_cast<Byte>(msb[byteIndex] | (1 << bit));
...
Bytes SidState::fullUpdate() const {
  ...
  writes.reserve(25);
  for (Byte r = 0; r <= 0x18; ++r)
    writes.push_back({r, reg[r]});
