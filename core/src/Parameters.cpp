...
ParamInfo cont(std::string id, std::string name, std::string group,
               std::uint16_t pos, Byte mask, Byte shift, int lo, int hi,
               int cc = -1) {
  return {std::move(id),
          std::move(name),
          std::move(group),
          DpAddress{pos, mask, shift},
          lo,
          hi,
...
std::vector<ParamInfo> build() {
  ...
  const std::vector<EnumChoice> lfoType = {{0, "Triangle"}, {1, "Saw"},
                                           {2, "Ramp"},     {3, "Pulse"},
                                           {4, "Random"},   {7, "Flat"}};
  ...
  const std::vector<EnumChoice> ctrlDest = {{0, "None"},
                                            {1, "LFO Depth"},
                                            {2, "LFO Speed"},
                                            {3, "LFO S/H"},
                                            {4, "LFO Lace"}};
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    add(t, enump("dctrl" + n + ".assign", "Direct Ctrl " + n + " Assign", g,
                 base, 0x7F, 0, 82));
    add(t, cont("dctrl" + n + ".limitUp", "Direct Ctrl " + n + " Limit Up", g,
                base + 1, 0x7F, 0, 0, 127));
    add(t, cont("dctrl" + n + ".limitDown", "Direct Ctrl " + n + " Limit Down",
                g, base + 2, 0x7F, 0, 0, 127));
  }
  ...
  add(t,
      boolp("filter.syncNoteOn", "Filter Sync To Note On", "Filter", 0x16, 4));
  ...
  // ---- Filter (0x18..0x23) ----------------------------------------------
  add(t, enump("filter.route", "Filter Route", "Filter", 0x18, 0x07, 0, 7, -1,
               filterRoute));
  add(t, cont("filter.resonance", "Filter Resonance", "Filter", 0x18, 0x0F, 4,
              0, 15));
  add(t, enump("filter.mode", "Filter Mode", "Filter", 0x19, 0x07, 0, 7, -1,
               filterMode));
  add(t, enump("filter.lfoToCutoff", "Filter LFO Source", "Filter", 0x19, 0x03,
               4, 3, -1, lfoSel));
  add(t,
      boolp("filter.forceReinit", "Force Note-On Re-Init", "Filter", 0x19, 6));
  add(t, cont("filter.cutoff", "Filter Cutoff", "Filter", 0x1A, 0x7F, 0, 0, 127,
              27));
  add(t, cont("filter.envDepth", "Filter Env Depth", "Filter", 0x1B, 0x7F, 0, 0,
              127, 28));
  add(t, cont("filter.envAttack", "Filter Env Attack", "Filter", 0x1C, 0x7F, 0,
              0, 127, 30));
  add(t, cont("filter.envDecay", "Filter Env Decay", "Filter", 0x1D, 0x7F, 0, 0,
              127, 31));
  add(t, cont("filter.envSustain", "Filter Env Sustain", "Filter", 0x1E, 0x7F,
              0, 0, 127, 32));
  add(t, cont("filter.envRelease", "Filter Env Release", "Filter", 0x1F, 0x7F,
              0, 0, 127, 33));
  add(t, cont("filter.lfoDepth", "Filter LFO Depth", "Filter", 0x20, 0x7F, 0, 0,
              127, 29));
  add(t, cont("filter.lfoWheelDepth", "Filter LFO Wheel Depth", "Filter", 0x21,
              0x7F, 0, 0, 127));
  ...
  add(t, cont("global.pitchSyncSpeed", "Pitch Sync Speed", "Global", 0x22, 0x7F,
              0, 50, 127));
  add(t, cont("global.pitchSyncHCut", "Pitch Sync HardCut", "Global", 0x23,
              0x0F, 0, 0, 15));

  // ---- Oscillators 1..3 --------------------------------------------------
  // Per-oscillator CC numbers (pages 38-39). Order matches the field list.
  struct OscCc {
    int arp, pitch, transpose, vibDepth, detune, porta, sync, ring, pwmStart,
        pwmAdd, pwmLfoDepth, delay, attack, decay, sustain, release;
  };
  const OscCc oscCc[3] = {
  ...
  for (int o = 0; o < 3; ++o) {
    ...
    // +0x01.. value bytes
    add(t, cont(p + "pitchTrack", g + " Pitch/Track", g, b + 0x01, 0x7F, 0, 0,
                99, c.pitch));
    add(t, cont(p + "arpSpeed", g + " Arp Speed", g, b + 0x02, 0x7F, 0, 0, 127,
                c.arp));
    add(t, bip(p + "transpose", g + " Transpose", g, b + 0x03, -24, 24,
               c.transpose));
    add(t, bip(p + "detune", g + " Detune", g, b + 0x04, -64, 63, c.detune));
    add(t, cont(p + "pitchBendRange", g + " Pitch Bend Range", g, b + 0x05,
                0x7F, 0, 0, 24));
    add(t, cont(p + "attack", g + " Attack", g, b + 0x06, 0x0F, 0, 0, 15,
                c.attack));
    add(t,
        cont(p + "decay", g + " Decay", g, b + 0x07, 0x0F, 0, 0, 15, c.decay));
    add(t, cont(p + "sustain", g + " Sustain", g, b + 0x08, 0x0F, 0, 0, 15,
                c.sustain));
    add(t, cont(p + "release", g + " Release", g, b + 0x09, 0x0F, 0, 0, 15,
                c.release));
    add(t,
        cont(p + "delay", g + " Delay", g, b + 0x0A, 0x7F, 0, 0, 127, c.delay));
    add(t, cont(p + "pwmStart", g + " PWM Start", g, b + 0x0B, 0x7F, 0, 0, 127,
                c.pwmStart));
    add(t, cont(p + "pwmAdd", g + " PWM Add", g, b + 0x0C, 0x7F, 0, 0, 127,
                c.pwmAdd));
    add(t, enump(p + "pwmLfo", g + " PWM LFO", g, b + 0x0D, 0x03, 0, 3, -1,
                 lfoSel));
    add(t, cont(p + "pwmLfoDepth", g + " PWM LFO Depth", g, b + 0x0E, 0x7F, 0,
                0, 127, c.pwmLfoDepth));
    // +0x0F OSC_WAVE byte: waveform in high nibble, SYNC=bit1, RINGM=bit2.
    add(t, enump(p + "waveform", g + " Waveform", g, b + 0x0F, 0x0F, 4, 15, -1,
                 wave));
    add(t, boolp(p + "sync", g + " Sync", g, b + 0x0F, 1, c.sync));
    add(t, boolp(p + "ringMod", g + " Ring Mod", g, b + 0x0F, 2, c.ring));
    add(t, cont(p + "portamento", g + " Portamento", g, b + 0x10, 0x7F, 0, 0,
                99, c.porta));
    add(t, enump(p + "vibratoLfo", g + " Vibrato LFO", g, b + 0x11, 0x03, 0, 3,
                 -1, lfoSel));
    add(t, cont(p + "vibratoDepth", g + " Vibrato Depth", g, b + 0x12, 0x7F, 0,
                0, 127, c.vibDepth));
    add(t, cont(p + "vibratoWheelDepth", g + " Vibrato Wheel Depth", g,
                b + 0x13, 0x7F, 0, 0, 127));
    add(t, cont(p + "tableSpeed", g + " Table Speed", g, b + 0x14, 0x7F, 0, 0,
                127));
  }

  // ---- LFOs 1..4 ---------------------------------------------------------
  struct LfoCc {
    int speed, sampHold, depth, lace, addDepth, fadeIn;
  };
  const LfoCc lfoCc[4] = {
      {88, 92, 89, 93, 90, 91},
  ...
  for (int l = 0; l < 4; ++l) {
    ...
    add(t,
        enump(p + "type", g + " Type", g, b + 0x00, 0x07, 0, 7, -1, lfoType));
    ...
    add(t, enump(p + "ctrlDest", g + " Ctrl Dest", g, b + 0x01, 0x07, 4, 4, -1,
                 ctrlDest));
    // +0x02.. value bytes
    add(t,
        cont(p + "speed", g + " Speed", g, b + 0x02, 0x7F, 0, 0, 127, c.speed));
    add(t, cont(p + "sampHold", g + " Sample & Hold", g, b + 0x03, 0x7F, 0, 0,
                127, c.sampHold));
    add(t,
        cont(p + "depth", g + " Depth", g, b + 0x04, 0x7F, 0, 0, 127, c.depth));
    add(t, enump(p + "addLfo", g + " Add LFO", g, b + 0x05, 0x03, 0, 3, -1,
                 lfoSel));
    add(t, cont(p + "laceSpeed", g + " Lace Speed", g, b + 0x06, 0x7F, 0, 0,
                127, c.lace));
    add(t, enump(p + "laceWith", g + " Lace With", g, b + 0x07, 0x07, 0, 4, -1,
                 laceWith));
    add(t, cont(p + "addDepth", g + " Add Depth", g, b + 0x08, 0x7F, 0, 0, 127,
                c.addDepth));
    add(t,
        cont(p + "ctrlValue", g + " Ctrl Value", g, b + 0x09, 0x7F, 0, 0, 127));
    add(t, cont(p + "fadeIn", g + " Fade In", g, b + 0x0A, 0x7F, 0, 0, 127,
                c.fadeIn));
  }
...
const ParamInfo *findParamById(const std::string &id) {
  ...
  auto it = std::find_if(t.begin(), t.end(),
                         [&](const ParamInfo &p) { return p.id == id; });
...
const ParamInfo *findParamByCc(int cc) {
  ...
  const auto &t = parameters();
  auto it = std::find_if(t.begin(), t.end(),
                         [&](const ParamInfo &p) { return p.cc == cc; });
  return it == t.end() ? nullptr : &*it;
}
