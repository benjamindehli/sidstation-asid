...
void Patch::setName(const std::string &n) {
  if (data.size() < kNameLength)
    data.resize(kNameLength, 0x20);
...
std::optional<Patch> decodePatchDump(const Bytes &msg) {
  ...
  if (msg.size() < 12)
    return std::nullopt;
  if (msg[6] != static_cast<Byte>(sysex::MessageType::PatchDump))
    return std::nullopt;
