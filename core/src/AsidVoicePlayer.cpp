...
void AsidVoicePlayer::reset() {
  ...
  for (int v = 0; v < 3; ++v) {
    ...
    sidState.setPulseWidth(
        v, 0x800); // 50% in case the waveform is changed to pulse
...
std::vector<Bytes> AsidVoicePlayer::start() {
  ...
  if (targetVoice >= 0 && targetVoice <= 2)
    ...
    for (int v = 0; v < 3; ++v)
      addVoice(v);
  for (Byte r = 0x15; r <= 0x18; ++r)
    w.push_back({r, sidState.reg[r]}); // filter + volume
...
std::vector<Bytes> AsidVoicePlayer::stop() {
  ...
  for (int v = 0; v < 3; ++v) {
    ...
    w.push_back(
        {static_cast<Byte>(SidState::voiceBase(v) + 4), sidState.control(v)});
  }
  return {encodeAsidUpdate(w)};
}

Bytes AsidVoicePlayer::frameForAction(const VoiceAction &a) {
  if (a.oscillator < 0 || a.oscillator > 2)
    return {};
  ...
  if (a.gateOn) {
    ...
    const double startNote =
        (nextGlideStart >= 0.0) ? nextGlideStart : a.midiNote;
    nextGlideStart = -1.0;
    sidState.setFrequency(a.oscillator,
                          sidFrequency(startNote + pitchOffset, clockHz));
    if (!a.retrigger) {
  ...
  const Byte gateLow =
      static_cast<Byte>(sidState.control(a.oscillator) & ~sid::kGate);
  sidState.setGate(a.oscillator, false);
  return encodeAsidDoubleControl(a.oscillator, gateLow, gateLow, {});
}

std::vector<Bytes>
AsidVoicePlayer::frameSequence(const std::vector<VoiceAction> &actions) {
...
std::vector<Bytes> AsidVoicePlayer::hardRestartFrames(int voice) const {
  ...
  // makes the unit apply the fast release so the envelope drains before the
  // note.
  const Byte fastRelease =
      static_cast<Byte>(sidState.reg[base + 6] & 0xF0); // keep sustain
...
std::vector<Bytes> AsidVoicePlayer::noteOn(int channel, int midiNote,
                                           int velocity) {
...
Bytes AsidVoicePlayer::setVolume(int vol0to15) {
  const Byte vol =
      static_cast<Byte>(vol0to15 < 0 ? 0 : (vol0to15 > 15 ? 15 : vol0to15));
...
Bytes AsidVoicePlayer::setCutoff(int cutoff0to2047) {
  const int c =
      cutoff0to2047 < 0 ? 0 : (cutoff0to2047 > 2047 ? 2047 : cutoff0to2047);
...
Bytes AsidVoicePlayer::setResonance(int res0to15) {
  const Byte res =
      static_cast<Byte>(res0to15 < 0 ? 0 : (res0to15 > 15 ? 15 : res0to15));
...
Bytes AsidVoicePlayer::setPulseWidth(int voice, int pw0to4095) {
  ...
  const int base = SidState::voiceBase(voice);
  return encodeAsidUpdate(
      {{static_cast<Byte>(base + 2), sidState.reg[base + 2]},
       {static_cast<Byte>(base + 3), sidState.reg[base + 3]}});
}

static Byte clampNibble(int v) {
  return static_cast<Byte>(v < 0 ? 0 : (v > 15 ? 15 : v));
}

Bytes AsidVoicePlayer::setAttackDecay(int voice, int attack0to15,
                                      int decay0to15) {
  if (voice < 0 || voice > 2)
    return {};
  sidState.setAttackDecay(voice, clampNibble(attack0to15),
                          clampNibble(decay0to15));
  const int base = SidState::voiceBase(voice);
  return encodeAsidUpdate(
      {{static_cast<Byte>(base + 5), sidState.reg[base + 5]}});
}

Bytes AsidVoicePlayer::setSustainRelease(int voice, int sustain0to15,
                                         int release0to15) {
  if (voice < 0 || voice > 2)
    return {};
  sidState.setSustainRelease(voice, clampNibble(sustain0to15),
                             clampNibble(release0to15));
  const int base = SidState::voiceBase(voice);
  return encodeAsidUpdate(
      {{static_cast<Byte>(base + 6), sidState.reg[base + 6]}});
}
...
Bytes AsidVoicePlayer::setFilterRouting(int voice, bool routeThroughFilter) {
  if (voice < 0 || voice > 2)
    return {};
  const Byte bit =
      voice == 0 ? sid::kFilt1 : (voice == 1 ? sid::kFilt2 : sid::kFilt3);
...
Bytes AsidVoicePlayer::setPitchTo(int voice, double fractionalNote) {
  if (voice < 0 || voice > 2)
    return {};
  sidState.setFrequency(voice,
                        sidFrequency(fractionalNote + pitchOffset, clockHz));
  const int base = SidState::voiceBase(voice);
  return encodeAsidUpdate(
      {{static_cast<Byte>(base + 0), sidState.reg[base + 0]},
       {static_cast<Byte>(base + 1), sidState.reg[base + 1]}});
...
Bytes AsidVoicePlayer::setPitchMod(int voice, double semitones) {
  if (voice < 0 || voice > 2 || currentNote[voice] < 0)
    return {};
