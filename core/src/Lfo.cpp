...
// One onWrap per boundary crossed. Bounded, and NaN-safe: the comparison rejects it
// before the cast, which would otherwise be undefined.
void Lfo::applyWraps(double n) {
    if (!(n > 0.0)) return;
    const int count = n >= static_cast<double>(kMaxWrapsPerStep)
                          ? kMaxWrapsPerStep : static_cast<int>(n);
    for (int i = 0; i < count; ++i) onWrap();
}

void Lfo::advance(double dtSeconds, double rateHz) {
    const double delta = dtSeconds * rateHz;
    if (!(delta > 0.0)) return;  // nothing to do, and this rejects NaN too
    // ph is always in [0, 1), so the integer part of the sum IS the number of cycle
    // boundaries this step crossed. The old test was "did the phase end up lower than
    // it started", which sees at most one boundary and misses them altogether when a
    // step covers a whole cycle and lands above where it began. That is reachable: the
    // modulation clock clamps a resumed dt to 4x its interval (160 ms at Eco) and the
    // rate goes to 20 Hz, so a single step can span 3.2 cycles.
...
void Lfo::setPhase(double cyclePosition) {
    ...
    if (!cycleLocked) { cycle = whole; cycleLocked = true; return; }
    const double crossed = whole - cycle;
    cycle = whole;
    // Forward only. A backward jump is the transport looping, and the old
    // phase-went-backwards test read that as a fresh cycle, so Sample & Hold re-rolled
    // its value on every pass of the loop instead of repeating it.
    applyWraps(crossed);
}
