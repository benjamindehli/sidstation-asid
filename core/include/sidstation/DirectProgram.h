...
//   F7
//
// The unit applies the write as:  byte = (byte & ~(mask<<shift)) | ((data & mask) << shift)
#pragma once
...
inline DecodedDirectProgram decodeDirectProgram(const Bytes& msg) {
    ...
    if (msg.size() != 13) return r;
    if (!sysex::hasValidInit(msg)) return r;
    if (msg[6] != static_cast<Byte>(sysex::MessageType::DirectProgram)) return r;
    if (msg.back() != sysex::kEnd) return r;
    r.address.position =
        static_cast<std::uint16_t>((static_cast<std::uint16_t>(msg[7] & 0x07) << 7) | (msg[8] & 0x7F));
