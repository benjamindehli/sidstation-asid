...
constexpr Byte kLowPass = 0x10, kBandPass = 0x20, kHighPass = 0x40,
               kVoice3Off = 0x80;

// Combine the four waveform toggles into the control-register waveform bits.
// The bits OR together, but on the 6581 noise silences/locks the others, so
// noise is exclusive. Returns 0 (a silent voice) when nothing is selected.
inline Byte waveformBits(bool triangle, bool saw, bool pulse, bool noise) {
  if (noise)
    return kNoise;
  return static_cast<Byte>((triangle ? kTriangle : 0) | (saw ? kSaw : 0) |
                           (pulse ? kPulse : 0));
}
...
struct SidState {
  ...
  void setControl(int voice, Byte control) {
    reg[voiceBase(voice) + 4] = control;
  }
  Byte control(int voice) const { return reg[voiceBase(voice) + 4]; }
  void setGate(int voice, bool on) {
    Byte &c = reg[voiceBase(voice) + 4];
    c = on ? static_cast<Byte>(c | sid::kGate)
           : static_cast<Byte>(c & ~sid::kGate);
  }
  ...
  void setAttackDecay(int voice, Byte attack, Byte decay) {
    reg[voiceBase(voice) + 5] =
        static_cast<Byte>((attack << 4) | (decay & 0x0F));
  }
  void setSustainRelease(int voice, Byte sustain, Byte release) {
    reg[voiceBase(voice) + 6] =
        static_cast<Byte>((sustain << 4) | (release & 0x0F));
  }
...
// robust release. This double-write path is applied reliably by the unit where
// a plain single control write is not. `otherWrites` carries any other
// registers.
