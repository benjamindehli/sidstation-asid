...
public:
  ...
  // Tempo sync: set the position from the song, in CYCLES (song position
  // divided by the note division), so 4.25 means "a quarter into cycle 4". The
  // fractional part becomes the phase; the integer part is kept as the cycle
  // index, which is what lets a cycle boundary be counted rather than guessed.
  // Crossing boundaries forward advances the random endpoints; jumping backward
  // (the transport looping) does not.
...
private:
  ...
  // A single step can cover many cycles (a resumed modulation clock at a high
  // rate, or a transport jump), so the wrap count is bounded: past this the
  // random endpoints are indistinguishable anyway, and it keeps the loop
  // finite.
  static constexpr int kMaxWrapsPerStep = 64;

  LfoShape shape = LfoShape::Sine;
  double ph = 0.0;
  // Tempo sync only: the last cycle index seen, so boundaries can be counted
  // from the song position instead of inferred from the phase going backwards.
  double cycle = 0.0;
  bool cycleLocked =
      false; // false until the first setPhase establishes a cycle
  unsigned int rng =
      0x1234567u; // deterministic, so the random shapes reproduce in tests
