...
// full sustain, voices unfiltered, master volume up) plus a few global
// controls. Richer per-voice editing comes later. Framework agnostic and
// tested.
#pragma once
...
public:
  ...
  // A harmless register re-write (sustain/release, unchanged) whose only
  // purpose is to be a following message: the unit applies a note-off's
  // gate-low only when another message arrives behind it, and after a release
  // the pitch stream has stopped, so nothing flushes it. Sent shortly after a
  // note-off, this does.
  Bytes settleFrame(int voice) const;

  // Hard restart, to dodge the 6581 ADSR bug on a fast re-attack: a slow
  // release leaves the envelope's shared rate counter high, and the next attack
  // then has to wrap it all the way around, which can stall the note into
  // silence. These two frames force a fast release with the gate held low so
  // the envelope drains to zero; the caller schedules them a few ms before the
  // note-on (the second frame flushes the first into effect on the
  // one-message-late unit), and the note-on itself restores the real
  // sustain/release. Does not touch the stored state, so the note-on rewrites
  // the true envelope registers.
  ...
  Bytes setWaveform(int voice,
                    Byte waveBits); // sid::kTriangle / kSaw / kPulse / kNoise
  ...
  Bytes
  setTest(int voice,
          bool on); // TEST bit: hold/reset the oscillator (silent while set)
  ...
  // Same write, but for an explicit fractional note rather than the sounding
  // one, so the caller can keep the pitch stream running when no note is held -
  // during a release tail, where the gate is already low but the envelope is
  // still fading. Without this the tail freezes at whatever pitch the vibrato
  // last wrote.
  ...
  void setNextGlideStart(double fractionalNote) {
    nextGlideStart = fractionalNote;
  }
...
private:
  ...
  int targetVoice = -1;
  int currentNote[3] = {-1, -1,
                        -1};    // sounding MIDI note per voice, for pitch mod
