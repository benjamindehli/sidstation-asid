...
public:
  ...
  void
  parentHierarchyChanged() override; // brand the standalone window's title bar
...
private:
  ...
  // built-up id ("wtNoise" + i, prefix + "Sync"), so each tick was ~150 map
  // walks and ~20 juce::String allocations. This is the message thread, so it
  // was never a glitch risk like the processor's - but the same fix costs
  // nothing here.
  struct ParamRefs {
  ...
  struct LfoControls {
    ...
    juce::ToggleButton syncButton{
        "BPM Sync"}; // under Rate: free Hz vs tempo division
    juce::ToggleButton wheelButton{
        "Mod Wheel"}; // under Depth: mod wheel scales the depth
  ...
  // A 2-segment switch bound to a 2-value choice parameter: the selected
  // segment lights up, clicking a segment sets the parameter.
  void setupSwitch(juce::ToggleButton &segA, juce::ToggleButton &segB,
                   const juce::String &labelA, const juce::String &labelB,
                   const juce::String &paramId);
  void setupLfo(juce::Component &parent, LfoControls &,
  ...
  static constexpr int kBorderY =
      22; // C64 screen border, top/bottom (centres the 496px screen)
  static constexpr int kPad = 8; // padding inside the dark screen, all sides
  static constexpr int kPresetBarH =
      34; // dark preset bar above the 720x540 content

  // The dark screen rectangle (kBg) and the padded area the controls live in.
  juce::Rectangle<int> screenBox() const {
    return getLocalBounds()
        .withTrimmedTop(kPresetBarH)
        .reduced(kBorder, kBorderY);
  }
  ...
  juce::Rectangle<int> innerArea() const {
    return screenBox()
        .reduced(kPad, 0)
        .withTrimmedTop(kPad - 3)
        .withTrimmedBottom(kPad);
  }

  SidLookAndFeel laf; // declared first so it outlives every child that uses it
  // Neutral dark look for the preset bar controls, since they sit on the dark
  // bar outside the C64 screen and should not be Commodore-styled.
  juce::LookAndFeel_V4 barLnF{juce::LookAndFeel_V4::getDarkColourScheme()};
  // Repaints a value-bar slider as the mouse moves over it, so its hover
  // preview (the muted delta at the would-be-click value) tracks the cursor.
  ...
  // The "in use" hint uses a BubbleMessageComponent (the same reliable popup
  // the knob value bubbles use); JUCE's TooltipWindow did not show inside the
  // DAW.
  ...
  // their own alpha. A per-voice "OverlayVoiceN.png" is used if present, else
  // "Overlay.png".
  juce::Image overlay[3];
  juce::Image overlayShared;
  // The overlay is a top child component (not paintOverChildren) so it sits
  // over the controls yet under the tooltip bubble, and passes all mouse events
  // through.
  ...
  // Hover hints via BubbleMessageComponent (JUCE's TooltipWindow did not show
  // in the DAW). Attach a control with add(); it shows the text on mouse-enter.
  struct Hints : juce::MouseListener {
    static constexpr int anySide =
        juce::BubbleComponent::above | juce::BubbleComponent::below |
        juce::BubbleComponent::left | juce::BubbleComponent::right;
    juce::BubbleMessageComponent bubble;
    juce::Component *host = nullptr;
    bool active =
        true; // the preset-bar "Tips" toggle; off = suppress hover hints
    struct Entry {
      juce::String enabled, disabled;
      int place;
    };
    std::map<juce::Component *, Entry> entries;
    // Enabled-only hint (says what the control does).
    void add(juce::Component &c, const juce::String &t,
             int placement = anySide) {
      entries[&c] = {t, {}, placement};
      c.addMouseListener(this, true); // true: also fires over child components
                                      // (e.g. a combo's label)
    }
    // Separate messages: what it does when enabled, why it is greyed when
    // disabled.
    ...
    void mouseEnter(const juce::MouseEvent &e) override {
      if (!active)
        return; // hints turned off from the preset bar
      ...
      for (; c != nullptr; c = c->getParentComponent())
        if ((it = entries.find(c)) != entries.end())
          break;
      if (it == entries.end() || host == nullptr)
        return;
      const bool dis = !c->isEnabled();
      const juce::String &t = (dis && it->second.disabled.isNotEmpty())
                                  ? it->second.disabled
                                  : it->second.enabled;
      if (t.isEmpty())
        return;
      ...
      bubble.showAt(host->getLocalArea(c, c->getLocalBounds()), msg, 0, false,
                    false);
    }
  ...
  // Top-right voice selector: three cells 1/2/3. This instance's voice is
  // bright. A voice another instance already drives shows in its (dim) hue and
  // is blocked (hovering it shows an "in use" bubble). A free voice is
  // monochrome and reveals its hue on hover, inviting a click to switch to it.
  struct VoiceSwitch : juce::Component {
    ...
    void paint(juce::Graphics &g) override {
      ...
      for (int i = 0; i < n; ++i) {
        juce::Rectangle<int> cell(
            i * w, 0, (i == n - 1 ? getWidth() - i * w : w), getHeight());
        ...
        if (on) {
          fill = c;
          txt = juce::Colour(SidLookAndFeel::kBg);
        } else if (hue) {
          fill = SidLookAndFeel::muted(c); // in use / hover
          txt = juce::Colour(SidLookAndFeel::kHot);
        } else {
          fill =
              juce::Colour(SidLookAndFeel::kBg).darker(0.55f); // idle: uniform
          txt = juce::Colour(SidLookAndFeel::kDim);
        }
        g.setColour(fill);
        g.fillRect(
            cell); // flush: no gap between cells, right cell meets the padding
        g.setColour(txt);
    ...
    void mouseDown(const juce::MouseEvent &e) override {
      ...
      if (usedByOther[i])
        return; // voice taken by another instance: blocked
      if (onSelect)
        onSelect(i);
    }
  };
  ...
  juce::TextButton oscTabBtn{"VOICE"}, ampModTabBtn{"MODULATION"},
      sharedTabBtn{"GLOBAL"}, waveTabBtn{"WAVETABLE"};
  ...
  juce::TextButton presetPrevBtn{"<"}, presetNextBtn{">"},
      presetSaveBtn{"Save"}, presetDeleteBtn{"Delete"};
  juce::TextButton tooltipToggle{
      "Tips"}; // preset-bar toggle for the hover hints
  ...
  // Tempo: an editable BPM field in standalone, a read-only host BPM in a DAW.
  // A hidden slider carries the APVTS "bpm" binding; bpmField is the visible
  // number.
  ...
  bool bpmHostDriven =
      false; // last known state, to toggle editability only on change
  ...
  juce::GroupComponent pitchModGroup, pwModGroup, filterGroup, cutModGroup,
      masterGroup;
  ...
  // by the step number box (accent fill while playing, dim beyond the table
  // length).
  struct StepIndicator : juce::Component {
    ...
    void paint(juce::Graphics &g) override {
      ...
      g.setColour(playing ? juce::Colour(SidLookAndFeel::kBg)
                          : juce::Colour(active ? SidLookAndFeel::kFg
                                                : SidLookAndFeel::kDim));
      g.setFont(SidLookAndFeel::mono());
  ...
  // plus a header over the arp stepper column.
  juce::Label wtWaveHead[4];
  juce::Label wtSyncHead{{}, "Syn"}, wtRingHead{{}, "Rin"},
      wtTestHead{{}, "Tst"}, wtPwHead{{}, "PW"}, wtArpHead{{}, "Arp"};
  ...
  juce::ToggleButton wtSyncTog[AsidProcessor::kWtSteps],
      wtRingTog[AsidProcessor::kWtSteps], wtTestTog[AsidProcessor::kWtSteps];
  std::unique_ptr<ButtonAtt> wtSyncAtt[AsidProcessor::kWtSteps],
      wtRingAtt[AsidProcessor::kWtSteps], wtTestAtt[AsidProcessor::kWtSteps];
  ...
  juce::TextButton wtArpDec[AsidProcessor::kWtSteps],
      wtArpInc[AsidProcessor::kWtSteps];
  ...
  // switches (a pair of toggle buttons each), stacked and grouped with glide
  // time.
  ...
  juce::ToggleButton filtExtButton{
      "External"}; // route the external audio input through the filter
  std::unique_ptr<ButtonAtt> filtExtAtt;
  juce::ToggleButton modeButtons[3]; // LP, BP, HP
  std::unique_ptr<ButtonAtt> modeAtts[3];
  int highlightedVoice =
      -1; // which filter checkbox is currently marked as ours
  juce::Slider cutoffKnob, resKnob, volumeKnob, latencyKnob;
  std::unique_ptr<SliderAtt> cutoffAtt, resAtt, volumeAtt, latencyAtt;
  juce::ToggleButton voice3offButton{
      "V3 Off"}; // V3 output off, used as mod source
