...
namespace {
juce::AudioParameterInt *intParam(const char *id, const char *name, int lo,
                                  int hi, int def) {
  return new juce::AudioParameterInt(juce::ParameterID{id, 1}, name, lo, hi,
                                     def);
}
...
AsidProcessor::makeLayout() {
  ...
  layout.add(std::make_unique<juce::AudioParameterBool>(
      juce::ParameterID{"waveTri", 1}, "Triangle", false));
  layout.add(std::make_unique<juce::AudioParameterBool>(
      juce::ParameterID{"waveSaw", 1}, "Sawtooth", true));
  layout.add(std::make_unique<juce::AudioParameterBool>(
      juce::ParameterID{"wavePulse", 1}, "Pulse", false));
  layout.add(std::make_unique<juce::AudioParameterBool>(
      juce::ParameterID{"waveNoise", 1}, "Noise", false));
  layout.add(std::unique_ptr<juce::AudioParameterInt>(
      intParam("attack", "Attack", 0, 15, 0)));
  layout.add(std::unique_ptr<juce::AudioParameterInt>(
      intParam("decay", "Decay", 0, 15, 0)));
  layout.add(std::unique_ptr<juce::AudioParameterInt>(
      intParam("sustain", "Sustain", 0, 15, 15)));
  layout.add(std::unique_ptr<juce::AudioParameterInt>(
      intParam("release", "Release", 0, 15, 0)));
  layout.add(std::unique_ptr<juce::AudioParameterInt>(
      intParam("pulseWidth", "Pulse Width", 0, 4095, 2048)));
  layout.add(std::unique_ptr<juce::AudioParameterInt>(
      intParam("coarse", "Coarse Tune", -24, 24, 0)));
  layout.add(std::unique_ptr<juce::AudioParameterInt>(
      intParam("fine", "Fine Tune", -50, 50, 0)));
  layout.add(std::unique_ptr<juce::AudioParameterInt>(
      intParam("pitchBendRange", "Pitch Bend Range", 0, 24, 2)));
  // Portamento: glide time in ms (0 = off), trigger, and stepped/smooth type.
  layout.add(std::unique_ptr<juce::AudioParameterInt>(
      intParam("portaTime", "Portamento", 0, 2000, 0)));
  ...
  layout.add(std::make_unique<juce::AudioParameterBool>(
      juce::ParameterID{"ring", 1}, "Ring Mod", false));
  // TEST bit: holds the oscillator in reset (silent) while on. Advanced.
  layout.add(std::make_unique<juce::AudioParameterBool>(
      juce::ParameterID{"test", 1}, "Test (osc reset)", false));
  ...
  layout.add(std::make_unique<juce::AudioParameterBool>(
      juce::ParameterID{"filt1", 1}, "Filter Voice 1", false));
  layout.add(std::make_unique<juce::AudioParameterBool>(
      juce::ParameterID{"filt2", 1}, "Filter Voice 2", false));
  layout.add(std::make_unique<juce::AudioParameterBool>(
      juce::ParameterID{"filt3", 1}, "Filter Voice 3", false));
  // External audio input through the filter (4th bit of the routing nibble).
  // Shared.
  layout.add(std::make_unique<juce::AudioParameterBool>(
      juce::ParameterID{"filtExt", 1}, "Filter External In", false));
  layout.add(std::make_unique<juce::AudioParameterBool>(
      juce::ParameterID{"modeLP", 1}, "Filter Low Pass", true));
  layout.add(std::make_unique<juce::AudioParameterBool>(
      juce::ParameterID{"modeBP", 1}, "Filter Band Pass", false));
  layout.add(std::make_unique<juce::AudioParameterBool>(
      juce::ParameterID{"modeHP", 1}, "Filter High Pass", false));
  layout.add(std::unique_ptr<juce::AudioParameterInt>(
      intParam("cutoff", "Cutoff", 0, 2047, 2047)));
  layout.add(std::unique_ptr<juce::AudioParameterInt>(
      intParam("resonance", "Resonance", 0, 15, 0)));
  layout.add(std::unique_ptr<juce::AudioParameterInt>(
      intParam("volume", "Volume", 0, 15, 15)));
  // Disconnects voice 3 from the output while it keeps running, so it can be
  // used purely as a ring/sync modulation source ($18 bit 7). Shared, only
  // affects V3.
  layout.add(std::make_unique<juce::AudioParameterBool>(
      juce::ParameterID{"voice3off", 1}, "Voice 3 Silent", false));
  ...
  layout.add(std::unique_ptr<juce::AudioParameterInt>(
      intParam("latency", "Output Latency", 0, 500, 0)));
  ...
  auto addLfo = [&layout](const juce::String &prefix,
                          const juce::String &name) {
    using C = juce::AudioParameterChoice;
    layout.add(std::make_unique<juce::AudioParameterBool>(
        juce::ParameterID{prefix + "On", 1}, name + " On", false));
    layout.add(std::make_unique<C>(
        ...
        0));
    layout.add(std::make_unique<juce::AudioParameterBool>(
        juce::ParameterID{prefix + "Sync", 1}, name + " Sync", false));
    ...
    layout.add(std::make_unique<juce::AudioParameterBool>(
        juce::ParameterID{prefix + "Wheel", 1}, name + " Mod Wheel", false));
    layout.add(std::make_unique<juce::AudioParameterInt>(
        juce::ParameterID{prefix + "Delay", 1}, name + " Delay", 0, 4000, 0));
  };
  ...
  layout.add(std::make_unique<Choice>(
      ...
      juce::StringArray{"Eco 25 Hz", "PAL 50 Hz", "NTSC 60 Hz", "HiFi 100 Hz"},
      1));

  // Tempo the BPM-synced LFOs lock to when there is no host transport
  // (standalone, or a host that reports no tempo). In a DAW the host BPM is
  // used instead and this is ignored. Saved per instance.
  layout.add(std::unique_ptr<juce::AudioParameterInt>(
      intParam("bpm", "Tempo (BPM)", 20, 300, 120)));
  ...
  layout.add(std::unique_ptr<juce::AudioParameterInt>(
      intParam("wtLoop", "WT Loop", 0, kWtSteps - 1, 0)));
  // Each step's waveform is combinable toggles, like the oscillator (noise is
  // exclusive in wtStepWaveBits). Triangle on by default matches the old
  // default.
  for (int i = 0; i < kWtSteps; ++i) {
    const juce::String s(i), n(i + 1);
    layout.add(std::make_unique<juce::AudioParameterBool>(
        juce::ParameterID{"wtTri" + s, 1}, "WT Tri " + n, true));
    layout.add(std::make_unique<juce::AudioParameterBool>(
        juce::ParameterID{"wtSaw" + s, 1}, "WT Saw " + n, false));
    layout.add(std::make_unique<juce::AudioParameterBool>(
        juce::ParameterID{"wtPulse" + s, 1}, "WT Pulse " + n, false));
    layout.add(std::make_unique<juce::AudioParameterBool>(
        juce::ParameterID{"wtNoise" + s, 1}, "WT Noise " + n, false));
    layout.add(std::make_unique<juce::AudioParameterBool>(
        juce::ParameterID{"wtSync" + s, 1}, "WT Sync " + n, false));
    layout.add(std::make_unique<juce::AudioParameterBool>(
        juce::ParameterID{"wtRing" + s, 1}, "WT Ring " + n, false));
    layout.add(std::make_unique<juce::AudioParameterBool>(
        juce::ParameterID{"wtTest" + s, 1}, "WT Test " + n, false));
    layout.add(std::make_unique<juce::AudioParameterInt>(
        juce::ParameterID{"wtPw" + s, 1}, "WT Pulse Width " + n, 0, 4095,
        2048));
    layout.add(std::make_unique<juce::AudioParameterInt>(
...
// Resolves every parameter the audio thread reads into a raw-value pointer,
// once. All the juce::String work and map lookups happen here, on the message
// thread, at construction. A jassert catches an id that does not exist, which
// would otherwise leave that parameter silently reading 0 for the life of the
// instance.
void AsidProcessor::buildParamPtrs() {
  ...
  pp.waveTri = at("waveTri");
  pp.waveSaw = at("waveSaw");
  pp.wavePulse = at("wavePulse");
  pp.waveNoise = at("waveNoise");
  pp.attack = at("attack");
  pp.decay = at("decay");
  pp.sustain = at("sustain");
  pp.release = at("release");
  ...
  pp.filt2 = at("filt2");
  pp.filt3 = at("filt3");
  pp.filtExt = at("filtExt");
  pp.modeLP = at("modeLP");
  pp.modeBP = at("modeBP");
  pp.modeHP = at("modeHP");
  pp.voice3off = at("voice3off");
  pp.cutoff = at("cutoff");
  pp.resonance = at("resonance");
  pp.volume = at("volume");
  pp.latency = at("latency");
  pp.modRate = at("modRate");
  pp.bpm = at("bpm");
  pp.wtOn = at("wtOn");
  ...
  // The three LFO targets share one parameter naming scheme, as do the
  // wavetable steps, so build those by prefix instead of listing every id.
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    lfos[i]->on = at(p + "On");
    lfos[i]->shape = at(p + "Shape");
    lfos[i]->sync = at(p + "Sync");
    lfos[i]->rate = at(p + "Rate");
    lfos[i]->div = at(p + "Div");
    lfos[i]->depth = at(p + "Depth");
    lfos[i]->wheel = at(p + "Wheel");
    lfos[i]->delay = at(p + "Delay");
  }
  for (int i = 0; i < kWtSteps; ++i) {
    ...
    s.tri = at("wtTri" + n);
    s.saw = at("wtSaw" + n);
    s.pulse = at("wtPulse" + n);
    s.noise = at("wtNoise" + n);
    s.sync = at("wtSync" + n);
    s.ring = at("wtRing" + n);
    s.test = at("wtTest" + n);
    s.pw = at("wtPw" + n);
...
double AsidProcessor::lfoAmount(const ParamPtrs::Lfo &ids, double nowMs) const {
  ...
  if (paramInt(ids.wheel))
    amt *= modWheelValue / 127.0; // wheel scales depth
  const int delayMs = paramInt(ids.delay);
  if (delayMs > 0)
    amt *= juce::jlimit(0.0, 1.0, (nowMs - noteOnMs) / delayMs); // fade in
  return amt;
}
...
void AsidProcessor::updatePitchOffset() {
  ...
  const double bend =
      (pitchWheelValue - 8192) / 8192.0 * paramInt(pp.pitchBendRange);
  asidPlayer.setPitchOffset(paramInt(pp.coarse) + paramInt(pp.fine) / 100.0 +
                            bend);
}
...
void AsidProcessor::applyControlChanges(int voice, bool forceAll) {
  ...
  if (!forceAll) {
    ...
    if (nowMs - lastControlMs < kControlIntervalMs)
      return; // coalesce rapid knob drags
    lastControlMs = nowMs;
  }
  ...
  // The wavetable owns the waveform register while it runs; leave it alone
  // then.
  ...
  // Notes go missing on the unit as decay and sustain rise, worst with both
  // at 15. Root cause unknown, and NOT established as a chip limitation: the
  // SidStation's own (non-ASID) mode plays high decay + sustain cleanly, so the
  // fault may well be in how this plugin sequences the ASID register writes -
  // the conditional hard restart in scheduleNotes keys off release alone and
  // ignores decay, which is one place to look. Until then this is a targeted
  // workaround, not a fix: at full sustain the decay is inaudible anyway (it
  // "decays" from the peak to the peak), so send 0 and keep the failure out of
  // the one case where it costs nothing. The editor greys the knob to match but
  // leaves the parameter alone, so the user's value survives.
  ...
  if (!lfoOwnedPw && !wtOwnsWave && (forceAll || pw != sent.pw)) {
    sent.pw = pw;
    flush(asidPlayer.setPulseWidth(voice, pw));
  }
  const int coarse = paramInt(pp.coarse), fine = paramInt(pp.fine),
            bendRange = paramInt(pp.pitchBendRange);
  if (forceAll || coarse != sent.coarse || fine != sent.fine ||
    ...
    flush(asidPlayer.setPitchMod(voice,
                                 0.0)); // retune a held note (empty if none)
  }
  // Skip the static sync/ring while the wavetable drives them per step.
  const int sync = paramInt(pp.sync);
  if (!wtOwnsWave && (forceAll || sync != sent.sync)) {
    sent.sync = sync;
    flush(asidPlayer.setSync(voice, sync != 0));
  }
  const int ring = paramInt(pp.ring);
  if (!wtOwnsWave && (forceAll || ring != sent.ring)) {
    sent.ring = ring;
    flush(asidPlayer.setRing(voice, ring != 0));
  }
  ...
  if (!wtOwnsWave && (forceAll || test != sent.test)) {
    sent.test = test;
    flush(asidPlayer.setTest(voice, test != 0));
  }
  ...
  if (forceAll || routing != sent.routing) {
    ...
    if (forceAll || routing != echoRouting.exchange(-1))
      reg17dirty = true;
  }
  if (forceAll || res != sent.resonance) {
    sent.resonance = res;
    if (forceAll || res != echoResonance.exchange(-1))
      reg17dirty = true;
  }
  ...
  if (forceAll || cutoff != sent.cutoff) {
    ...
    if (cutoffFree && (forceAll || cutoff != echoCutoff.exchange(-1))) {
      flush(asidPlayer.setCutoff(cutoff));
      globalSent = true;
    }
  }
  ...
  if (forceAll || mode != sent.mode) {
    ...
    const Byte modeBits = static_cast<Byte>(
        (mode & 1 ? sid::kLowPass : 0) | (mode & 2 ? sid::kBandPass : 0) |
        (mode & 4 ? sid::kHighPass : 0) | (mode & 8 ? sid::kVoice3Off : 0));
    if (forceAll || mode != echoMode.exchange(-1)) {
      flush(asidPlayer.setFilterMode(modeBits));
      globalSent = true;
    }
  }
  ...
  if (forceAll || vol != sent.volume) {
    sent.volume = vol;
    if (forceAll || vol != echoVolume.exchange(-1)) {
      flush(asidPlayer.setVolume(vol));
      globalSent = true;
    }
  }
...
double AsidProcessor::sampleLfo(sidstation::Lfo &lfo, const ParamPtrs::Lfo &ids,
                                double dt, bool playing, double ppq,
                                double bpm) {
  lfo.setShape(static_cast<sidstation::LfoShape>(
      juce::jlimit(0, 6, paramInt(ids.shape))));
  if (paramInt(ids.sync) != 0) {
    const double beats = beatsForDivision(paramInt(ids.div));
    if (playing)
      lfo.setPhase(ppq / beats); // locked to the song
    else
      lfo.advance(dt, (bpm / 60.0) /
                          beats); // free-run at the synced rate when stopped
  } else {
...
// Every modulation source runs on one clock and its register changes go out in
// a single combined ASID frame per tick, so the SidStation (which applies ~one
// ASID update per SID frame) never gets more frames than it can use. Running
// many mods with separate frames overran that limit and starved some registers.
void AsidProcessor::updateModulation(int voice, bool blockHasNotes) {
  ...
  if (auto *ph = getPlayHead()) {
    if (const auto pos = ph->getPosition()) {
      ...
      if (const auto q = pos->getPpqPosition())
        ppq = *q;
      if (const auto b = pos->getBpm(); b && *b > 0.0) {
        bpm = *b;
        hostHasBpm = true;
      }
      const double srr = juce::jmax(1.0, getSampleRate());
      if (const auto s = pos->getTimeInSamples())
        blockPlayheadMs = *s * 1000.0 / srr;
      else if (const auto t = pos->getTimeInSeconds())
        blockPlayheadMs = *t * 1000.0;
    }
  }
  // Standalone (or a host that reports no tempo): lock the synced LFOs to the
  // user BPM param. Expose the host BPM to the editor: 0 means "none", so it
  // shows an editable field; a positive value is the fixed host tempo it
  // displays read-only.
  const bool useParamBpm = wrapperType == wrapperType_Standalone || !hostHasBpm;
  if (useParamBpm)
    bpm = static_cast<double>(paramInt(pp.bpm));
  hostBpmValue.store(useParamBpm ? 0.0 : bpm, std::memory_order_relaxed);
  // On the block where the transport just stopped, scheduleNotes releases the
  // held note. Streaming this voice's control register here would carry the
  // still-high gate and, scheduled at now+latency, land after that release and
  // re-gate the voice (with two voices the per-instance timing differs, so one
  // hangs). Skip it.
  ...
  // belongs to. Stopped: now. Playing: the block's playhead mapped to wall
  // time.
  const double nowMs = juce::Time::getMillisecondCounterHiRes();
  ...
  if (playing) {
    ...
    if (ahead >= -50.0 && ahead <= kMaxScheduleAheadMs)
      sendTimeMs = aligned;
  }

  const int curNote = asidPlayer.currentNoteOf(voice);

  // Park the envelope once the fade has finished (see parkAtMs). Checked every
  // block, not just on a modulation tick, and before the idle early-return
  // below - a voice with nothing modulating is exactly the one that needs
  // parking. The frames are the same pair the pre-roll drain uses: fast
  // release, then the write that commits it. Neither touches the stored
  // registers, so the next note-on restores the real release, which it always
  // writes as part of its attack frame.
  if (parkPending && curNote < 0 && nowMs >= parkAtMs) {
    parkPending = false;
    for (const auto &f : asidPlayer.hardRestartFrames(voice))
      sendAsid(f);
  }
  ...
  // through the filter. With no voice filtered the cutoff changes nothing, so
  // the stream would be wasted MIDI bandwidth. Routing is shared, so any routed
  // voice (not just this instance's) counts.
  const bool filterInUse = routingMask() != 0;
  if (paramInt(pp.cutLfo.on) && cutDepth > 0 && filterInUse)
    AsidShared::get().claimCutoffMod(this);
  else
    AsidShared::get().releaseCutoffMod(this);
  const bool cutOn =
      cutDepth > 0 && filterInUse && AsidShared::get().isCutoffModOwner(this);
  if (!cutOn && lfoOwnedCutoff)
    sent.cutoff = -1;
  ...
  // with no modulation. A quiet voice needs the steady flow both so a busy
  // voice can't starve it and, as testing showed, so a glide plays correctly
  ...
  if (blockHasNotes || !anyMod || modStopTransition) {
    modTickMs = 0.0;
    return;
  }
  if (modTickMs > 0.0 && nowMs - modTickMs < interval)
    return;
  double dt = (modTickMs <= 0.0)
                  ? interval
                  : juce::jmin(nowMs - modTickMs, 4.0 * interval);
  dt /= 1000.0;
  modTickMs = nowMs;
  ...
  auto addReg = [&](int reg) {
    writes.push_back({static_cast<sidstation::Byte>(reg), sid.reg[reg]});
  };
  ...
  if (wtActive) {
    if (const int step = wtPlayer.currentStep(); step >= 0 && step < kWtSteps) {
      ...
      asidPlayer.setWaveform(
          voice, static_cast<sidstation::Byte>(wtStepWaveBits(step)));
  ...
  // value. Written as an absolute note so the release tail (no note held, so no
  // curNote to offset from) can use the same path and keep its vibrato moving.
  if (pitchOn) {
    double soundingNote;
    if (curNote >= 0) {
      if (gliding) {
        const double step =
            (12.0 / (portaTimeMs / 1000.0)) * dt; // ms per octave
        if (glidePitch < curNote)
          glidePitch =
              std::min(static_cast<double>(curNote), glidePitch + step);
        else
          glidePitch =
              std::max(static_cast<double>(curNote), glidePitch - step);
      }
      soundingNote = (paramInt(pp.portaType) == 1)
                         ? std::round(glidePitch)
                         : glidePitch; // stepped glide
    } else {
      soundingNote =
          releaseTailNote; // fading: pitch is fixed, vibrato still moves
    }
    const double vibrato =
        vibratoOn ? sampleLfo(pitchStream, pp.pitchLfo, dt, playing, ppq, bpm) *
  ...
  if (pwOn) {
    ...
    asidPlayer.setPulseWidth(
        voice, juce::jlimit(0, 4095,
                            paramInt(pp.pulseWidth) +
                                static_cast<int>(
                                    v * lfoAmount(pp.pwLfo, nowMs) * 2047.0)));
    addReg(base + 2);
    addReg(base + 3);
  }
  ...
  if (cutOn) {
    ...
    asidPlayer.setCutoff(juce::jlimit(
        0, 2047,
        paramInt(pp.cutoff) +
            static_cast<int>(v * lfoAmount(pp.cutLfo, nowMs) * 2047.0)));
...
static const char *kSharedIds[] = {
    "cutoff", "resonance", "volume", "latency", "modRate", "filt1",    "filt2",
    "filt3",  "filtExt",   "modeLP", "modeBP",  "modeHP",  "voice3off"};
...
AsidProcessor::AsidProcessor()
  ...
  for (const char *id : kSharedIds)
    apvts.addParameterListener(id, this);
  // Track this instance's voice so the editor can mark/block voices already in
  // use.
  ...
  if (AsidShared::get().hasData.load())
    sharedUpdated();
...
void AsidProcessor::parameterChanged(const juce::String &id, float value) {
  // Track which SID voice this instance drives, so the editor can mark and
  // block voices already taken by another instance.
  if (id == "asidVoice") {
    AsidShared::get().setClientVoice(
        this, juce::jlimit(0, 2, juce::roundToInt(value)));
    return;
  }
  if (!AsidShared::isShared(id))
    return;
  auto &sh = AsidShared::get();
  // Ignore an echo of a value we just synced in from another instance.
  if (static_cast<int>(std::lround(value)) == sh.valueFor(id))
    return;
  // The user changed a shared control here: publish it to the other instances.
  sh.publish(paramInt(pp.cutoff), paramInt(pp.resonance), modeMask(),
             routingMask(), paramInt(pp.volume), paramInt(pp.latency),
             paramInt(pp.modRate), this);
}
...
void AsidProcessor::resetVoiceToDefault() {
  for (auto *p : getParameters())
    if (auto *wid = dynamic_cast<juce::AudioProcessorParameterWithID *>(p))
      if (isVoiceSoundParam(wid->paramID) &&
          apvts.getParameter(wid->paramID) != nullptr)
...
juce::File AsidProcessor::presetsDir() const {
  auto dir =
      juce::File::getSpecialLocation(juce::File::userApplicationDataDirectory)
          .getChildFile("DehliMusikk")
          .getChildFile("SidStation ASID")
...
juce::StringArray AsidProcessor::presetNames() const {
  ...
  for (const auto &f :
       presetsDir().findChildFiles(juce::File::findFiles, false, "*.xml"))
...
bool AsidProcessor::savePreset(const juce::String &name) {
  ...
  // parameter is fine here - unlike the audio thread, which uses the cached
  // pointers.
  ...
  for (auto *p : getParameters())
    if (auto *wid = dynamic_cast<juce::AudioProcessorParameterWithID *>(p))
      if (isVoiceSoundParam(wid->paramID) &&
          apvts.getParameter(wid->paramID) != nullptr)
        tree.setProperty(wid->paramID, rawById(wid->paramID),
                         nullptr); // raw value
  const auto xml = tree.createXml();
  // Only claim the preset once it is actually on disk, so a failed write does
  // not leave the editor displaying a name that nothing can load.
  if (xml == nullptr || !xml->writeTo(presetFile(key)))
    return false;
...
bool AsidProcessor::loadPreset(const juce::String &name) {
  ...
  if (!tree.isValid())
    return false;
  resetVoiceToDefault(); // params absent from an older preset fall back to
                         // default
  for (int i = 0; i < tree.getNumProperties(); ++i) {
    const auto id = tree.getPropertyName(i).toString();
    if (!isVoiceSoundParam(id))
      continue;
...
void AsidProcessor::sharedUpdated() {
  ...
  echoCutoff.store(sh.cutoff.load());
  setParamValue("resonance", sh.resonance.load());
  echoResonance.store(sh.resonance.load());
  setParamValue("volume", sh.volume.load());
  echoVolume.store(sh.volume.load());
  setParamValue("latency", sh.latency.load());
  setParamValue("modRate",
                sh.modRate.load()); // shared modulation clock, no hardware echo
...
void AsidProcessor::sendAsid(const Bytes &asidMessage, double sendTimeMs) {
  if (asidMessage.size() < 2)
    return;
  const double t = (sendTimeMs < 0.0) ? juce::Time::getMillisecondCounterHiRes()
                                      : sendTimeMs;
  AsidShared::get().addBytes(
      static_cast<int>(asidMessage.size())); // for the load meter
...
void AsidProcessor::addFrame(juce::MidiBuffer &out, const Bytes &frame,
                             int samplePos) {
  if (frame.size() < 2)
    return;
  AsidShared::get().addBytes(
      static_cast<int>(frame.size())); // for the load meter
  out.addEvent(juce::MidiMessage::createSysExMessage(
                   frame.data() + 1, static_cast<int>(frame.size()) - 2),
               samplePos);
}

void AsidProcessor::scheduleNotes(const juce::MidiBuffer &midiMessages,
                                  int voice) {
  ...
  const bool justStopped =
      !playing && lastPlaying; // transport stopped or paused
  lastPlaying = playing ? 1 : 0;
  lastPlayheadMs = blockPlayheadMs;

  if (playing)
    AsidShared::get().reportPlayOffset(blockPlayheadMs - nowMs);
  ...
  if (justStopped) {
    ...
    // instance whose processBlock the host stopped calling, so it never runs
    // its own release; whichever instance still gets this stop block releases
    // them all (deduped, so overlapping stop-panics do not overrun the unit).
    // releaseGen makes each instance clear its stale note on resume.
    AsidShared::get().panicOnStop();
    glidePitch = -1.0;
    lastGateOffMs =
        nowMs; // a forced release still leaves the ADSR counter parked
    releaseTailUntilMs =
        -1.0e18; // hard gate-off, so there is no fade to follow
    parkPending = false;
  }
  ...
  for (const auto meta : midiMessages) {
    ...
    if (on) {
      ...
      freshAttack =
          !wasHeld; // nothing sounding = a real attack, not a legato overlap
      if (freshAttack)
        noteOnMs = nowMs; // restart the LFO fade-in on a fresh attack
      const bool always = paramInt(pp.portaTrigger) == 1; // 0 Legato, 1 Always
      const bool glide = paramInt(pp.portaTime) > 0 && glidePitch >= 0.0 &&
                         (always || wasHeld);
      if (glide)
        asidPlayer.setNextGlideStart(glidePitch); // start at the held pitch
      else
        glidePitch = m.getNoteNumber(); // jump straight to the note
      ...
      if (paramInt(pp.wtOn))
        asidPlayer.setWaveform(
            voice, static_cast<sidstation::Byte>(wtStepWaveBits(0)));
    }

    double target = juce::jmax(eventMs, voiceClockMs);

    // Hard restart: drain the envelope to zero, with the gate held low and the
    // release forced to 0, before a fresh attack whose ADSR counter could still
    // be parked at a large value (see kAdsrWrapMs). Two frames, because the
    // second commits the first on the one-message-late unit; the note-on itself
    // rewrites the real release.
    //
    ...
    // long decay and release 0. There the generator sits in the long decay
    // phase while the note sounds, so the counter is large even though the
    // release is the fastest there is - and the old `rel > 0` guard skipped
    // that case entirely. PRE-ROLL ONLY: the note itself must never be moved.
    // noteOn mutates the stored SID state right here, while the frame carrying
    // it is scheduled for `target`, and updateModulation broadcasts that SAME
    // control register on its own clock from the stored state. Delay the note
    // past its own modulation frames and they gate the voice on early, after
    // which the drain's gate-low cuts it: heard as a short burst and then
    // silence. So the drain only ever uses room that already exists ahead of
    // the note - the host's render lookahead, or an Output Latency the user has
    // dialled in - and is skipped when there is none.
    if (freshAttack && (target - lastGateOffMs) <
                           sidReleaseMs(paramInt(pp.release)) + kAdsrWrapMs) {
      const double room = target - juce::jmax(nowMs, voiceClockMs);
      const auto hr = (room >= kHardRestartMinMs)
                          ? asidPlayer.hardRestartFrames(voice)
                          : std::vector<sidstation::Bytes>{};
      if (hr.size() == 2) {
        // A partial drain is still worth having: it is the waiting that
        // matters, and a short wait beats none. Never runs past the note.
        const double drain =
            juce::jmin(room, kHardRestartFlushMs + kHardRestartMs);
        const double drainAt = target - drain;
        addFrame(out, hr[0], posOf(drainAt));
        addFrame(
            out, hr[1],
            posOf(drainAt + juce::jmin(kHardRestartFlushMs, drain * 0.25)));
      }
    }

    const auto frames =
        on ? asidPlayer.noteOn(ch, m.getNoteNumber(), m.getVelocity())
           : asidPlayer.noteOff(ch, m.getNoteNumber());
    const int posMs = posOf(target);
    for (const auto &f : frames)
      addFrame(out, f, posMs);
    voiceClockMs = target;
    ...
    if (off) {
      ...
      if (fellBackTo >= 0) {
        if (paramInt(pp.portaTime) == 0)
          glidePitch = fellBackTo;
      } else {
        lastGateOffMs =
            target; // fully released; times the next attack's hard restart
  ...
  if (!out.isEmpty())
    midi().sendScheduled(out, nowMs, 1000.0);
}
...
void AsidProcessor::processBlock(juce::AudioBuffer<float> &buffer,
  ...
  // If the shared watchdog released our voice while our processBlock was
  // stalled (the host stopped calling it), clear the stale note here on resume
  // so we do not re-gate it before the transport's own notes play.
  if (const int rg = AsidShared::get().releaseGen(voice);
      rg != lastReleaseGen) {
    lastReleaseGen = rg;
    if (asidPlayer.currentNoteOf(voice) >= 0) {
      ...
      // counter parked exactly as a normal release would. Record it, or the
      // next attack skips its hard restart and can come out silent.
  ...
  if (const int gen = AsidShared::get().outGeneration.load();
      gen != lastOutGeneration) {
  ...
  if (initRequest.exchange(false)) {
    asidPlayer.reset();
    for (const auto &msg : asidPlayer.start()) {
      sendAsid(msg);
      sendAsid(msg);
    }
    forceControls =
        true; // push the current control values after the start state
  }
  ...
  for (const auto meta : midiMessages) {
    ...
    if (m.isNoteOn() || m.isNoteOff())
      ...
      pitchWheelValue = m.getPitchWheelValue();
      updatePitchOffset();
    } else if (m.isController() && m.getControllerNumber() == 1)
      modWheelValue = m.getControllerValue();
  }
  updateModulation(voice, blockHasNotes);

  scheduleNotes(midiMessages, voice);

  // Heartbeat for the shared stuck-note watchdog: report our voice's held note
  // (or -1). If we stop reporting (the host stops calling processBlock) while a
  // note is held, the watchdog thread releases it - the one context that stays
  // alive when a track is unselected and the transport stops.
...
juce::AudioProcessorEditor *AsidProcessor::createEditor() {
  return new AsidEditor(*this);
}

void AsidProcessor::getStateInformation(juce::MemoryBlock &destData) {
  auto state = apvts.copyState();
  state.setProperty("currentPreset", currentPresetName,
                    nullptr); // for the editor's display
  state.setProperty("showTooltips", tooltipsOn,
                    nullptr); // editor "Tips" toggle
  // The MIDI output is shared by all instances, so every instance stores the
  // same device; the name is a fallback for when an identifier changes between
  // sessions.
...
void AsidProcessor::setStateInformation(const void *data, int sizeInBytes) {
  if (auto xml = getXmlFromBinary(data, sizeInBytes)) {
    ...
    if (outId.isNotEmpty() && midi().outputIdentifier() != outId) {
      const bool ok =
          midi().openOutputByIdentifier(outId) ||
          (outName.isNotEmpty() && midi().openOutputMatching(outName));
      if (ok)
        AsidShared::get().outGeneration.fetch_add(1); // make instances re-push
    }
  }
}

juce::AudioProcessor *JUCE_CALLTYPE createPluginFilter() {
  return new AsidProcessor();
}
