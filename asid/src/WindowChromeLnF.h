...
// default yellow/red. Only the title bar and its buttons are overridden; the
// rest keeps the V4 look. Applied to the standalone DocumentWindow only (a DAW
// owns its own title bar) via each editor's parentHierarchyChanged().
#pragma once
...
struct WindowChromeLnF : juce::LookAndFeel_V4 {
  ...
  juce::Button *createDocumentWindowButton(int buttonType) override {
    ...
    const float a = 0.32f,
                b = 0.68f; // glyph inset, so it stays small and centred
    juce::String name;
    ...
    auto *btn = new juce::ShapeButton(name, col, col.brighter(0.4f),
                                      col.brighter(0.8f));
...
// Leaked deliberately, like SidLookAndFeel::c64Typeface() and
// AsidShared::get(): a static LookAndFeel destroyed by __cxa_finalize at
// Logic's quit tears down cached JUCE font and colour state on top of
// frameworks that are already finalizing.
