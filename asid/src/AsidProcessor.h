...
public:
  ...
  MidiHub &midi() {
    return AsidShared::get().out;
  } // one output shared by all instances
  int wtStep() const {
    return wtStepDisplay.load(std::memory_order_relaxed);
  } // playing step, -1 = none
  // Host tempo the editor should display: >0 is the DAW BPM (shown read-only),
  // 0 means no host transport (standalone), so the editor shows an editable
  // field.
  double hostBpm() const {
    return hostBpmValue.load(std::memory_order_relaxed);
  }
  // True if another instance is currently driving SID voice v (0..2). The
  // editor's voice switch marks voices it shares live with another instance.
  ...
  // volume (they belong to every voice) and this instance's identity
  // (asidVoice).
  void resetVoiceToDefault();

  // Voice-sound presets (the same parameter set Init resets), stored as XML
  // files in a user folder. Not the shared filter/volume, voice selection, or
  // tempo.
  ...
  // juce::File::getChildFile honours "../" and absolute paths, so an
  // unsanitised name could write outside the presets folder and then never show
  // up in the list. createLegalFileName drops the separators that make that
  // reachable, and is idempotent, so applying it twice is harmless.
...
private:
  ...
  //
  // The audio thread must not build a juce::String (it always heap-allocates)
  // or walk the APVTS parameter map (a std::map<StringRef>, so string compares
  // per lookup). The old paramInt("id") and paramInt(prefix + "Suffix") calls
  // did both, roughly 30 allocations per modulation tick at up to 100 Hz once
  // the wavetable and all three LFOs were live.
  //
  // Safe to hold: APVTS creates one heap ParameterAdapter per parameter during
  // its own construction and never rebuilds them, and replaceState only swaps
  // the ValueTree, so these stay valid across setStateInformation.
  //
  // Named members rather than an id-keyed table, so a mistake is a compile
  // error instead of a parameter that silently reads 0 forever.
  struct ParamPtrs {
    ...
    struct Lfo {
      P on{}, shape{}, sync{}, rate{}, div{}, depth{}, wheel{}, delay{};
    };
    Lfo pitchLfo, pwLfo, cutLfo;
    struct Step {
      P tri{}, saw{}, pulse{}, noise{}, sync{}, ring{}, test{}, pw{}, arp{};
    };
    Step wt[kWtSteps];
  };
  ParamPtrs pp;
  ...
  void addFrame(juce::MidiBuffer &out, const sidstation::Bytes &frame,
                int samplePos);
  ...
  int routingMask() const {
    return (paramInt(pp.filt1) ? 1 : 0) | (paramInt(pp.filt2) ? 2 : 0) |
           (paramInt(pp.filt3) ? 4 : 0) |
           (paramInt(pp.filtExt)
                ? 8
                : 0); // bit 3: external input through the filter
  }
  int modeMask() const {
    return (paramInt(pp.modeLP) ? 1 : 0) | (paramInt(pp.modeBP) ? 2 : 0) |
           (paramInt(pp.modeHP) ? 4 : 0) |
           (paramInt(pp.voice3off)
                ? 8
                : 0); // bit 3: voice 3 output off (silent mod source)
  }
  ...
  int waveBits() const {
    return sidstation::sid::waveformBits(
        paramInt(pp.waveTri), paramInt(pp.waveSaw), paramInt(pp.wavePulse),
        paramInt(pp.waveNoise));
  }
  // Same, for one wavetable step's four toggles (noise exclusive).
  int wtStepWaveBits(int step) const {
    if (step < 0 || step >= kWtSteps)
      return 0;
  ...
  // wheel when its Mod Wheel toggle is on, and ramped in over its fade-in
  // delay.
  double lfoAmount(const ParamPtrs::Lfo &ids, double nowMs) const;
  int pitchWheelValue =
      8192;              // last MIDI pitch wheel value (14-bit, centre 8192)
  int modWheelValue = 0; // last MIDI mod wheel (CC 1) value, 0..127
  double noteOnMs =
      -1.0e18; // last note attack time (wall clock), for the LFO fade-in
  // Currently-playing wavetable step (-1 = not playing), for the editor's
  // display.
  ...
  std::atomic<bool> initRequest{
      true}; // send full state on first block / device open
  int lastOutGeneration =
      0; // shared-device generation this instance has pushed for

  // Note scheduling state (this instance's single voice).
  static constexpr double kMaxScheduleAheadMs =
      500.0; // sane alignment ceiling (> lookahead)
  static constexpr double kSettleMs =
      15.0; // trailing flush after a note-off under pitch mod
  // 6581 ADSR delay bug, which is what drops notes at high decay/sustain. The
  // envelope generator compares a 15-bit LFSR counter against a period picked
  // from the current phase's rate nibble; if the phase or rate changes while
  // the counter has already passed the new period, the counter must run its
  // whole 32767-count sequence before the envelope moves. 32767 / 985248 Hz =
  // up to ~33 ms at PAL. A long decay or release parks the counter at large
  // values, so the failure tracks decay and sustain rather than release.
  static constexpr double kAdsrWrapMs = 33.3;
  // Drain window before a re-attack: the worst-case wrap plus the ~6 ms the
  // fastest release itself takes, so by gate-on the envelope really is at zero
  // AND the counter is cycling on a short period. The old 16 ms was under the
  // wrap alone, which is why the muting was intermittent - the counter's phase
  // at gate-off is effectively random, so it sometimes drained in time and
  // sometimes did not.
  static constexpr double kHardRestartMs = kAdsrWrapMs + 8.0;
  // Gap between the fast-release write and the control re-write that commits it
  // on the one-message-late unit.
  static constexpr double kHardRestartFlushMs = 5.0;
  // Below this much room ahead of the note there is no point draining: the two
  // frames would land on top of the note-on rather than clearing the way for
  // it.
  static constexpr double kHardRestartMinMs = 12.0;
  double voiceClockMs = 0.0; // target time of the last frame sent, keeps order
  double lastGateOffMs =
      -1.0e18;         // when this voice last released, to time hard restarts
  int lastPlaying = 0; // transport state last block, to spot a start
  int lastModPlaying =
      0; // transport state last block in updateModulation, to spot a stop
  double lastPlayheadMs = 0.0; // playhead last block, to spot a jump
  int lastReleaseGen =
      0; // shared watchdog release generation seen, to clear a stale note
  juce::String
      currentPresetName;  // last saved/loaded preset, for the editor's display
  bool tooltipsOn = true; // editor "Tips" toggle, persisted in plugin state
  // A voice-sound parameter (in a preset, and reset by Init): not a shared
  // global, not the voice selection, not the tempo.
  ...
  double glidePitch =
      -1.0; // current sounding pitch (fractional note); -1 = no note
  // Release tail. The gate is low but the envelope is still fading, so the
  ...
  // finished, the release nibble is forced to 0: inaudible, since the envelope
  // is already at zero, but it leaves the counter cycling on a SHORT period, so
  // the next attack needs no wrap however long the gap. Where the pre-roll
  // drain needs room ahead of a note, this uses the time after one, of which
  // there is plenty.
  //
  ...
  std::atomic<int> echoCutoff{-1}, echoResonance{-1}, echoMode{-1},
      echoRouting{-1}, echoVolume{-1};

  // Last control values sent, for change detection on the audio thread. routing
  // and mode are 3-bit masks (routing: voice 1/2/3; mode: LP/BP/HP,
  // combinable).
  struct Sent {
    int voice = -1, wave = -1, attack = -1, decay = -1, sustain = -1,
        release = -1;
    int pw = -1, sync = -1, ring = -1, test = -1, routing = -1, coarse = -100,
        fine = -100;
